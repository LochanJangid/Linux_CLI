...
def pwd(commands):
    print("Thats me", os.getcwd())
    return os.getcwd()

def cd(commands):
    if incomplete_command(commands): return None

    if commands[1] == '..': os.chdir(os.path.dirname(os.getcwd()))
    else:
        try: os.chdir(os.path.join(os.getcwd(), commands[1]))
...
def mkdir(commands):
    ...
    for folder in folders:
        try: 
            ...
            print(f'mkdir: cannot create "{folder}": File exists')
...
def rm(commands):
    ...
    if len(commands) > 2 and commands[2] not in options:
